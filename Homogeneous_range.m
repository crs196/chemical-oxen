function [ ETA, IMEP, NOX_ppm, TU, TB] = Homogeneous_range(RPM, varargin)
close all
...
if ( nargin == 0 )
    ...
    function [ dCOdt ] = carbonmonoxide(T)
        % calculate rate of CO formation d[CO]/dt given
        % inputs:
        % T [K] : gas mixture temperature, kelvin
        % P [bar] : cylinder pressure, bar
        % y [...] : equilibrium mole fraction of constituents
        % CO [mol/cm^3] : current CO concentration
        % outputs:
        % dCOdt [ (mol/cm^3) / sec ] : rate of CO formation
        % extended zeldovich rate constants from Heywood Table 11.1 (cm^3/mol-s)
        kCO = (6.76e10)*exp(T/1102);
