...
[Temp,pressure,theta_test]=FiniteHeatRelease();
...
Pressure=pressure*100; % Pressure is now in kPa
...
dvdT_gamma=zeros(length(Temp),1);
dvdP_gamma=zeros(length(Temp),1);
...
%% Plot Gamma v. Crank Angle
    %theta_plot=linspace(-180,180,length(Temp));
    figure()
