...
for i = 1:NN
    [fy, vol, ht,hflux,UU] = integrate_ht(theta,thetae,fy);
    %print values
...
set(gca,'fontsize',19,'linewidth',1.5,'Xlim', [-180 180]);
...
ylabel('Heat flux q{"} (MW/m^2)','fontsize',18);
    ...
    function[fy,vol,ht,hflux,UU] = integrate_ht(theta,thetae,fy)
        ...
        function [yprime,U] = rates(theta,fy)
...
%mass flow
thetaS=save.theta;
cr = 0.04450898; %crank radius "cr" (m)
len = 0.13335; %connecting rod length (m)
X = (s*cr)*cos(thetaS)+ sqrt((len.^2)-(cr.^2)*(sin(thetaS).^2));
d = R/(P_crank*100.*T2); %density
Area = (save.vol)./X(1:end-1)'; %volume/stroke
m = d*UU.*Area'; %mass flow after combusion for engine, will need compression scale for after turbo
m_turbo = m*7.55986;
% fprintf('Mass Flow of Engine= %5.2f\n', m);
% fprintf('Mass Flow after Turbocharger= %5.2f\n', m_turbo);
